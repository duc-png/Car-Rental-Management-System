...
public class OwnerServiceImpl implements OwnerService {
    ...
    public List<OwnerProfileResponse> listOwners(Integer page, Integer size, String q) {
        ...
        // ROLE OWNER (đổi theo enum của bạn)
        Page<UserEntity> owners = (q == null || q.trim().isEmpty())
                ? userRepository.findByRole(UserRole.OWNER, pageable)
                : userRepository.searchByRole(UserRole.OWNER, q.trim(), pageable);
...
//        if (owner.getRoleId() != UserRole.OWNER) {
...
//        }
    ...
    }// làm tròn chữ số thập phân => vd: v = 4.34 → 4.34*10=43.4 → round = 43 → /10=4.3; v = 4.35 → 43.5 → round = 44 → /10=4.4
}
