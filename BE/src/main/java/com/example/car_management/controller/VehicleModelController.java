...
public class VehicleModelController {
    ...
    public ResponseEntity<ApiResponse<List<VehicleModelResponse>>> list() {
        ...
        List<VehicleModelResponse> data = models.stream()
                .map(model -> VehicleModelResponse.builder()
                        ...
                        .build())
                .sorted(Comparator
                        .comparing((VehicleModelResponse item) -> item.getBrandName() == null ? "" : item.getBrandName(), String.CASE_INSENSITIVE_ORDER)
    ...
    public ResponseEntity<ApiResponse<VehicleModelResponse>> create(@Valid @RequestBody CreateVehicleModelRequest req) {
        ...
        if (existing == null) {
            ...
            String finalTypeName = (typeName == null || typeName.isBlank()) ? DEFAULT_CAR_TYPE_NAME : typeName;
            CarTypeEntity type = carTypeRepository.findByNameIgnoreCase(finalTypeName)
                    .orElseGet(() -> carTypeRepository.save(CarTypeEntity.builder().name(finalTypeName).build()));
            ...
            // Nếu model đã tồn tại nhưng type đang là Unknown -> cho phép cập nhật type khi client gửi typeName.
            String existingTypeName = existing.getType() != null ? normalize(existing.getType().getName()) : null;
            boolean existingUnknown = existingTypeName == null || existingTypeName.isBlank() || DEFAULT_CAR_TYPE_NAME.equalsIgnoreCase(existingTypeName);
            boolean incomingValid = typeName != null && !typeName.isBlank() && !DEFAULT_CAR_TYPE_NAME.equalsIgnoreCase(typeName);
            if (existingUnknown && incomingValid) {
                CarTypeEntity type = carTypeRepository.findByNameIgnoreCase(typeName)
                        .orElseGet(() -> carTypeRepository.save(CarTypeEntity.builder().name(typeName).build()));
