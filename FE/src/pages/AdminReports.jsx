...
export default function AdminReports() {
    ...
    const [selectedJourneyBookingId, setSelectedJourneyBookingId] = useState(null)
    ...
    useEffect(() => {
        if (!token) return
        setLoading(true)
        const loader = tab === 'revenue' ? loadRevenue : loadStats
        loader().finally(() => setLoading(false))
    }, [tab, fromDate, toDate, granularity, token, loadRevenue, loadStats])

    return (
        <section className="admin-reports-page">
            <header className="reports-header">
                <div>
                    <p className="reports-breadcrumb">Quản trị / Báo cáo</p>
            ...
            <div className="reports-tabs">
                ...
                <button
                    type="button"
                    className={tab === 'bookingDetail' ? 'active' : ''}
                    onClick={() => setTab('bookingDetail')}
                >
                    Chi tiết booking
                </button>
            </div>
            ...
            <div className="reports-content">
                        ...
                        {revenue && (
                            <>
                                <div className="revenue-summary">
                                    ...
                                    <p className="period">
                                        {revenue.periodFrom} → {revenue.periodTo}
                                    </p>
                                </div>
                ...
                {!loading && tab === 'stats' && (
                    <div className="report-card">
                        ...
                        {stats && (
                            <>
                                <p className="period">
                                    {stats.periodFrom} → {stats.periodTo} (theo{' '}
                ...
                {!loading && tab === 'bookingDetail' && (
                    <div className="report-card">
                        <h2>Tra cứu chi tiết booking</h2>
                        <p className="period">Xem toàn bộ hành trình từ đặt xe tới thanh toán phí phạt.</p>
                        <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                            ...
                            <button
                                ...
                                className="preset-btn"
                                onClick={() => {
                                    const id = Number(bookingLookupId)
                                    if (!id) {
                                        toast.error('Vui lòng nhập Booking ID hợp lệ')
                                        return
                                    }
                                    setSelectedJourneyBookingId(id)
                                }}
                            >
                                Xem chi tiết
                            </button>
            ...
            {selectedJourneyBookingId && (
                <BookingJourneyModal
                    bookingId={selectedJourneyBookingId}
                    onClose={() => setSelectedJourneyBookingId(null)}
                />
            )}
        </section>
    )
